...
OUT_DIR_BASE="/var/tmp/famfold/test/minifold_48L_r${RECYCLE}_${MODE}_out"
...
CMD=(uv run python predict.py "$FASTA" --out_dir "$OUT_DIR_BASE" --cache "$CACHE" --model_size 48L --token_per_batch 2048 --num_recycling "$RECYCLE")
if [ -n "$OVERRIDE" ]; then
