...
def get_atom_coords(
    ...
    all_atom_loss_mask = np.zeros(
        [num_res, residue_constants.atom_type_num], dtype=np.float32
    )
    for res_index in range(num_res):
        pos = np.zeros([residue_constants.atom_type_num, 3], dtype=np.float32)
        mask = np.zeros([residue_constants.atom_type_num], dtype=np.float32)
        loss_mask = np.zeros([residue_constants.atom_type_num], dtype=np.float32)
        ...
        if not res_at_position.is_missing:
            ...
            for atom in res.get_atoms():
                ...
                if atom_name in residue_constants.atom_order.keys():
                    ...
                    loss_mask[residue_constants.atom_order[atom_name]] = 1.0
                    # loss_mask[residue_constants.atom_order[atom_name]] = float(
                    #     bfactor >= 50.0
                    # )
                    ...
                    loss_mask[residue_constants.atom_order["SD"]] = 1.0
                    # loss_mask[residue_constants.atom_order["SD"]] = float(
                    #     bfactor >= 50.0
                    # )

        all_atom_positions[res_index] = pos
        all_atom_mask[res_index] = mask
        all_atom_loss_mask[res_index] = loss_mask
    ...
    return all_atom_positions, all_atom_mask, all_atom_loss_mask
