...
def send_to_pdb(coords, af_seq, mask, batch_idx, out=None, idx_freq=500, prefix="", ca_only=True, add_sc=False, n=1):
    if out is None:
        return
    if idx_freq == -1 or batch_idx % idx_freq == 0:
        if n == -1:
            n = coords.shape[0]
        with torch.no_grad():
            coords = coords.float().detach().cpu().numpy()
            af_seq = af_seq.detach().cpu().numpy()
            ...
            for ii in range(n):
                ...
                path_out = os.path.join(out, prefix + "_b{}.pdb".format(ii))
                pdb_string = output_to_pdb(af_seq_i, coords_i, mask_i, ca_only=ca_only, add_sc=add_sc)
                with open(path_out, "w") as f:
                    f.write(pdb_string)


def output_to_pdb(af_seq, coords, mask, ca_only=True, add_sc=False):
    """Returns the pbd (file) string from the model given the model output."""

    L = len(af_seq)
    if ca_only:
        coords = np.pad(coords[:, None], ((0, 0), (1, 1), (0, 0)))
        mask = np.pad(mask[:, None], ((0, 0), (1, 1)))
        b_factors = np.zeros((L, 1))
    else:
        if add_sc:
            # mask = np.stack([mask] * 14).T
            b_factors = np.zeros((L, 14))
            seq = "".join([restype_order_with_x_inverse[aa] for aa in af_seq])
            mask = torch.tensor(np.array([SUPREME_INFO[aa]['cloud_mask'] for aa in seq]))
            print("mask", mask.shape)
            print("coords", coords.shape)
        else:
            mask = np.stack([mask] * 3).T
            b_factors = np.zeros((L, 3))
    pred = Protein(
        aatype=af_seq,
        atom_positions=coords,
        atom_mask=mask,
        residue_index=1 + np.arange(L),
        b_factors=b_factors,
    )
    return to_pdb(pred)
